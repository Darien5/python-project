...
class ArrayList(List):
    ...
    def add(self, i : int, x : object) :
        '''
            ...
            Inputs:
                ...
                x: Object type, i.e., any object
        '''
        if self.n == len(self.a): self.resize(self)
        if i < self.n /2:
            self.j = (self.j - 1) % len(self.a)
            for v in self.a[self.n:self.n-1, i-1]:
                self.a[(self.j + v) % len(self.a)] = self.a[(self.j + v + 1) % len(self.a)]
        else:
            for v in self.a[self.n:self.n-1, i +1]:
                self.a[(self.j + v) % len(self.a)] = self.a[(self.j + v + 1) % len(self.a)]
    ...
    def remove(self, i : int) -> object:
        ...
        if i < (self.n / 2):
            for v in self.a[i, i-1: 1]:
                self.a[(self.j+v) % len(self.a)] = self.a[(self.j + v + 1) % len(self.a)]
        else:
            for v in self.a[i,i + 1:self.n -2]:
                self.a[(self.j + v) % len(self.a)] = self.a[(self.j + v + 1) % len(self.a)]
        self.n -= 1
